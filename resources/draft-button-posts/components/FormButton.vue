...
export default {
  ...
  mounted() {
    if (!this.field.isDraft) {
      ...
      if (positionDraftButton && positionDraftButton !== this.$refs.createDraftButtonPosts) {
        this.actionButton.parentNode.removeChild(positionDraftButton);
      }
      this.actionButton.parentNode.append(this.$refs.createDraftButtonPosts);
    }
  },
